CREATE DATABASE IF NOT EXISTS Shops_cats;
...
VALUES 
        ...
        ("Zuza",4,3);
-- 1.Вывести всех 
-- котиков по магазинам по id (условие соединения shops.id = cats.shops_id)
SELECT * FROM shops;
...
-- 2. Вывести
--  магазин, в котором продается кот “Мурзик” (попробуйте выполнить 2 способами)
...
JOIN cats
ON shops.id = cats.shops_id
WHERE cats.name NOT IN ("Murzik", "Zuza") AND cats.id != '3';


------- Последнее задание, таблица:

CREATE DATABASE IF NOT EXISTS Analysis_groups_orders;

USE Analysis_groups_orders;
...
SELECT an_name, an_price, DATE(`ord_datetime`)
FROM analysis
JOIN orders
ON orders.ord_an = analysis.an_id
WHERE `ord_datetime` BETWEEN 
DATE("2020-02-05") AND DATE_ADD('2020-02-05', INTERVAL 8 DAY)
