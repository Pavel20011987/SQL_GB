...
SELECT DISTINCT MARK, COLOR, COUNT(*) AS 'count' FROM auto
WHERE MARK IN ("BMW")
GROUP BY MARK, COLOR
UNION
SELECT DISTINCT MARK, COLOR, COUNT(*) AS 'count' FROM auto
...
SELECT DISTINCT MARK, (SELECT COUNT(*) FROM auto T where T.MARK != A.MARK) AS 'Количество авто не этой марки' 
FROM auto A;
...
(30),
(50);
Напишите запрос, который вернет строки 
из таблицы test_a, 
...
SELECT t1.* 
FROM test_a AS t1 LEFT JOIN test_b AS t2 
    ON t1.id = t2.id
WHERE t2.id IS NULL
